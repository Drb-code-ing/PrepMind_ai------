...
| ChatRunBudget 合同          | `@repo/types`、Prisma schema/migration、owner-scoped repository、Worker reservation/settlement/terminal reconcile、显式 UNCERTAIN recovery、dispatch 单胜者、终态 guard、`@repo/agent` typed budget port 与真实单 ledger 并发验收已实现 | 隔离脚本尚待 Docker 恢复后执行；产品 Agent stage 注入、Trace 对账和真实模型结算未完成 |
...
2. 恢复 Docker 后执行 ChatRunBudget 隔离跨节点/crash recovery 脚本并封存证据；随后补齐其他 Agent stage 和 Trace 对账（ticket 05）。
