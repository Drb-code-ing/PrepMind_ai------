...
| ChatRunBudget 合同          | `@repo/types`、Prisma schema/migration、owner-scoped repository、Worker reservation/settlement/terminal reconcile、显式 UNCERTAIN recovery、dispatch 单胜者、终态 guard、`@repo/agent` typed budget port 与隔离 PostgreSQL 并发/crash recovery 验收已实现 | 产品 Agent stage 注入、Trace 对账和真实模型结算未完成 |
...
2. 继续补齐其他 Agent stage 和 Trace 对账（ticket 05）；ChatRunBudget 隔离跨节点/crash recovery 证据已封存。
