...
| Retriever / FinalResponse   | Worker 的 `rag_answer` 路径已调用 owner-bound Retriever projection，并将结果送入 deterministic Verifier | 不能据此证明上游每个 Agent 或 SLA；Verifier model candidate、embedding/provider 生产证据仍待验证 |
...
2. 继续补齐其他 Agent stage 和 Trace 对账（ticket 05），复用 Server turn-bound stage runner；Router、Retriever Worker RAG 调用和 deterministic Verifier 已完成，下一步接入 Verifier model candidate。
