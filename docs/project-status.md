...
stage wiring；最终回答 generator 仍是 deterministic baseline，也不是真实模型 Worker。
...
| Router / Verifier           | Router 与 Verifier 均进入 Server Worker stage 路径；确定性安全门优先，Verifier 可选 DeepSeek candidate                 | 产品真实模型 smoke、跨节点 ledger/Trace 对账仍待完成 |
...
2. 继续补齐其他 Agent stage 和 Trace 对账（ticket 05），复用 Server turn-bound stage runner；Router、Retriever Worker RAG 调用和 Verifier model-stage wiring 已完成，下一步接入 FinalResponse 独立预算与真实模型 generator。
