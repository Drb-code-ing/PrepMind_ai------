...
| Retriever / FinalResponse   | `/api/chat` 主回答链有真实模型 smoke；Server Worker 尚未接入 owner-bound Retriever projection                         | 不能据此证明上游每个 Agent 或 SLA；Verifier 必须等待真实 chunks 输入 |
...
2. 继续补齐其他 Agent stage 和 Trace 对账（ticket 05），复用 Server turn-bound stage runner；Router 已完成 Server stage 接入，下一步先迁移 Retriever，
