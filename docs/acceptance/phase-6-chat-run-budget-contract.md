...
2. 扩展 Retriever/Verifier/Tutor/FinalResponse 的 Agent stage 接入。Server 已完成 owner-bound Retriever projection，并在 Worker 的 `rag_answer` 路径调用；仍需验证 embedding/provider 并将真实 chunks 交给 Verifier。复用 Server turn-bound runner，结算真实 usage/cost，并与 terminal Outbox、Redis stream、Trace 做 bounded reconciliation。Router 仍需独立产品 smoke。
