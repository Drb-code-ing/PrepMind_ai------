...
| KnowledgeVerifierAgent            | 审查 Retriever evidence 的可信度                                                                                  | Server Worker stage 先 deterministic；可选 DeepSeek `deepseek-v4-pro` candidate                          | `KNOWLEDGE_VERIFIER_MODEL_ENABLED` + 全局 live + 知识专用 key + exact base URL；`1 / 1600 / 400`；4.5s | 只读 owner-bound chunks；不得放宽安全边界或写知识库；输出 `trusted/suspicious/conflict/insufficient`            | conservative fallback；不可确认调用保留 dispatched hold；本轮无专属 product-live 证据                                                  | 跨节点 ledger/Trace 对账及真实产品 smoke 仍缺                                        |
